...
# Development builds can keep Python build dependencies isolated under the
# firmware build directory instead of modifying the system Python install.
_local_pydeps = Path(__file__).resolve().parents[1] / 'Firmware' / 'build' / 'pydeps'
if _local_pydeps.is_dir():
    sys.path.insert(0, str(_local_pydeps))
...
try:
    ...
    if ex.name == 'cantools':
        raise SystemExit(
            'Missing Python package cantools. Install it with: '
...
import inspect
if 'choices' not in inspect.signature(can.Signal).parameters:
    from cantools.database.conversion import BaseConversion
    _Signal = can.Signal

    def _legacy_signal(name, start, length, **kwargs):
        scale = kwargs.pop('scale', 1)
        offset = kwargs.pop('offset', 0)
        choices = kwargs.pop('choices', None)
        is_float = kwargs.pop('is_float', False)
        kwargs['conversion'] = BaseConversion.factory(
            scale=scale, offset=offset, choices=choices, is_float=is_float)
        return _Signal(name, start, length, **kwargs)

    can.Signal = _legacy_signal
